...
;; fixtures
;;
(def train-vec-3 [{:init-row 4, :vx -1, :vy 0, :init-col 7, :length 1, :id-stem :tr-1}
                  {:init-row 5, :vx 1, :vy 0, :init-col 7, :length 1, :id-stem :tr-2}
                  {:init-row 6, :vx 1, :vy 0, :init-col 7, :length 1, :id-stem :tr-3}])
...
(t/deftest init-train
  (t/testing "init-train"
    ...
    (dispatch-sync [:cube-test.frig-frog.events/init-train
                     {:id-stem :train-1 :vx -1 :vy 0 :length 1 :init-col 7 :init-row 4}])
    (let [r-db @db/app-db]
      ; (prn "r-db=" r-db)
    ...
    ;; add a second train
    (dispatch-sync [:cube-test.frig-frog.events/init-train {:id-stem :train-2 :length 3}])
    (let [r-db @db/app-db]
      ; (prn "r-db=" r-db)
      (t/is (contains? r-db :trains))
...
(t/deftest drop-train-idx
  (t/testing "drop-train-idx"
    ...
    (let [r-db @db/app-db
      ...
      (t/is (contains? r-db :trains))
      (t/is (= (count trains) 2))
      (t/is (= (:id-stem (get trains 0)) :tr-1))
      (t/is (= (:id-stem (get trains 1)) :tr-3)))))

(t/deftest drop-train-id-stem
  (t/testing "drop-train-id-stem"
    (dispatch-sync [:cube-test.events/seed-db {:trains train-vec-3}])
    (dispatch-sync [::ff-events/drop-train-id-stem :tr-2])
    (let [r-db @db/app-db
          trains (:trains r-db)]
      (prn "r-db=" r-db)
      (t/is (contains? r-db :trains))
...
(t/deftest get-train-by-id-stem
  (t/testing "get-train-by-id-stem"
    (dispatch-sync [:cube-test.events/seed-db {:trains train-vec-3}])
    (let [r-db @db/app-db
          trains (:trains r-db)
          ; train (dispatch-sync [::ff-events/get-train-by-id trains :tr-2])
          train (ff-train/get-train-by-id-stem trains :tr-2)]
     (prn "***train=" train)
     (t/is (not (nil? train)))
     (t/is (= (:id-stem train) :tr-2)))))
...
(t/deftest update-train-id-stem
  (t/testing "update-train-id-stem"
    (dispatch-sync [:cube-test.events/seed-db
                     {:trains train-vec-3}])
    (prn "hi")
    (dispatch-sync [::ff-events/update-train-id-stem :tr-2 {:length 3}])
    (prn "hi2")
    ; (dispatch-sync [::ff-events/drop-train-idx 1])
    (let [r-db @db/app-db
          trains (:trains r-db)]
...
(comment
 ...
 (t/test-vars [#'drop-train-id-stem])
 (t/test-vars [#'get-train-by-id-stem])
 (t/test-vars [#'update-train-id-stem]))
