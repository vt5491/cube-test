...
(defn hemisferic-loaded [ac]
  ;; (println "hemisferic-loaded: loaded-scene=" loaded-scene)
  (.createRootMesh ac)
  (.addAllToScene ac)
  (set! hemisferic-asset-container ac)
  (let [root (.getMeshByName main-scene/scene "assetContainerRootMesh")]
    (prn "hemi-load:  root=" root)
...
(defn append-hemisferic [path file]
  ...
  (let [cb #(do
              ...
              (init-exp-gui)
              (main-scene-overrides))]
    (.LoadAssetContainer bjs/SceneLoader path file main-scene/scene cb)))
